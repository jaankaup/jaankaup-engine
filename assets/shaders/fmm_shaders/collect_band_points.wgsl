/// Kernel that finds all fmm band cells. 
/// The number of found cells is stored to fmm_fmm_counter[1]. The fmm_counter[1] must be equal to zero. 
...
fn main(@builtin(local_invocation_id)    local_id: vec3<u32>,
	...
	if (local_index == 0u) {
	    offset = atomicAdd(&fmm_counter[1], shared_counter);
        }
        storageBarrier();
        //workgroupBarrier();
