...
fn main(@builtin(local_invocation_id)    local_id: vec3<u32>,
	...
	while (items_to_process > 0u) {
	    workgroupBarrier();
            ...
            var chunks = udiv_up_safe32(items_to_process, 256u);

	    for (var i: u32 = 0u; i < chunks ; i = i + 1u) {
                workgroupBarrier();

	        var actual_index = local_index + i * 256u; // + buffer_swap_id * buffer_offset;
	    ...
	    workgroupBarrier();
            ...
            chunks = udiv_up_safe32(items_to_process, 256u);

            // Update the active list values to the actual fim data.
	    for (var i: u32 = 0u; i < chunks ; i = i + 1u) {
                workgroupBarrier();

	        var actual_index = local_index + i * 256u;
	        var swap_index = actual_index + buffer_swap_id * buffer_offset;
